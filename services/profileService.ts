...
export const ensureCoupleCode = async (userId: string): Promise<string> => {
    ...
    const { data, error } = await supabase.from('profiles').select('couple_code').eq('id', userId).single();
    if (data?.couple_code) return data.couple_code;

    // Gerar novo código único
    const code = Math.random().toString(36).substring(2, 8).toUpperCase();
    await supabase.from('profiles').upsert({ id: userId, couple_code: code }, { onConflict: 'id' });
    return code;
};

    ...
    <div className="min-h-screen bg-white selection:bg-rose-100">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-6 overflow-hidden">
        ...
        <div className="max-w-5xl w-full text-center space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-1000">
          ...
          <p className="text-xl md:text-2xl text-slate-500 max-w-3xl mx-auto font-medium leading-relaxed">
            Mais do que um desafio de 60 dias, o Amor em Ação é o check-up diário da saúde do seu casamento. Cultive ciclos infinitos de conexão, intimidade e aventura.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-8">
            ...
            >
              Salvar Meu Casamento Agora <ChevronRight className="w-6 h-6 group-hover:translate-x-1 transition-transform" />
            </button>
            <a href="#plans" className="px-10 py-6 bg-white text-slate-900 border border-slate-200 rounded-[2.5rem] font-bold text-xl hover:bg-slate-50 transition-all">
              Ver Planos
            </a>
          </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-rose-950/20 backdrop-blur-xl animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[3.5rem] shadow-2xl overflow-hidden p-10 md:p-14">
            <div className="text-center space-y-4 mb-10">
              <h3 className="text-3xl font-black text-rose-950">Comece Agora</h3>
              <p className="text-slate-500 font-medium italic">Sua jornada de 7 dias grátis começa aqui.</p>
            </div>
            
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); onLogin(name || 'Usuário'); }}>
              ...
              <button className="w-full py-6 bg-rose-600 text-white rounded-[2rem] font-black text-xl hover:bg-rose-700 transition-all shadow-xl shadow-rose-100 mt-6">
                Ativar Teste Grátis
              </button>
            </form>
            
            <button onClick={() => setShowAuth(false)} className="w-full mt-6 text-slate-400 font-bold text-sm hover:text-rose-600">
      ...
      <section className="py-24 px-6 bg-emerald-50">
        <div className="max-w-4xl mx-auto bg-white p-12 md:p-20 rounded-[4rem] border-4 border-emerald-100 text-center space-y-8 shadow-2xl">
          ...
          <button 
            onClick={() => setShowAuth(true)}
            className="px-12 py-6 bg-emerald-600 text-white rounded-[2.5rem] font-black text-xl hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-100"
          >
        ...
        <div className="max-w-7xl mx-auto space-y-20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {plans.map((plan) => (
              <div 
                key={plan.id}
                className={`relative bg-white p-12 rounded-[4rem] shadow-xl border-2 transition-all duration-500 hover:-translate-y-4 ${
                  plan.id === 'quarterly' ? 'border-rose-500 scale-105 z-10 shadow-rose-100' : 'border-white hover:border-rose-100'
                }`}
              >
                {plan.tag && (
                  <div className={`absolute -top-5 left-1/2 -translate-x-1/2 text-white text-[10px] font-black uppercase tracking-widest py-2 px-6 rounded-full shadow-lg ${
                    plan.id === 'annual' ? 'bg-emerald-600' : 'bg-rose-600'
                  }`}>
                    {plan.tag}
                  </div>
                )}
